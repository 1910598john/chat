        ...
        if ($row['sentby'] == $chosen) {
            ...
            echo '  <div class="message-wrapper">
                <div id="message'.$row['id'].'" class="message '.$row['sentby'].'" style="align-items:center;display:flex;'.$alignment.';margin:10px 0;">
                    ...
                    <div style="max-width:40%;overflow-wrap:break-word;background:'.$bgc.';color:'.$font_color.';padding:10px;border-radius:10px;">'.stripslashes($row['message']).'</div>
                </div>
            </div>';
        }
        else {
            ...
            echo '  <div class="message-wrapper">
                    <div id="message'.$row['id'].'" class="message '.$row['sentby'].'" style="align-items:center;display:flex;'.$alignment.';margin:10px 0;">
                        <div style="max-width:50%;overflow-wrap:break-word;background:'.$bgc.';color:'.$font_color.';padding:10px;border-radius:10px;">'.stripslashes($row['message']).'</div>
                    </div>
                </div>';
        }
